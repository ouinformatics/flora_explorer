...
function mongoQuery(settings) {
    var outData = [];
    query = JSON.stringify(settings.query)
    $.getJSON('http://test.cybercommons.org/mongo/db_find/' + settings.db + '/' + settings.col + '/' + query + '/?callback=?', 
        function(data) {
...
$('#plot').bind("plotselected", function(event, ranges) {
    $.plot($('#plot'), getData(ranges.xaxis.from, ranges.xaxis.to), $.extend(true, {}, options, { xaxis: {min:ranges.xaxis.from, max:ranges.xaxis.to} }));
});
...
$('#variable').change( function () { updateUI(); } );
